...
// router.get('/api/v1/customers/:customer_id', loginRequired, updateUser) // get user info PRIVATE
...
export const getUserInfo = (req, res) => {
  ...
  User.find({ _id: userId })
    .exec((err, user) => {
      if (err) {
        res.status(400).json({ 
          ERROR: 'Request failed. Check user id'
        })
      } else {
        res.json(user)
      }
...
export const updateUserById = (req, res) => {
    ...
    const user_id = { _id: req.params.customer_id }
